...
listNode* push(listNode* top, char* item) {
	listNode* newNode = (listNode*)malloc(sizeof(listNode));
	strcpy_s(newNode->data ,4,item);
	newNode->link = top;
	top = newNode;
...
void insertFirstNode(linkedList_h *L, char *x) {
	...
	strcpy_s(newNode->data,4, x);						// 새 노드의 데이터 필드에 x 저장  
	newNode->link = L->head;
	L->head = newNode;
}
